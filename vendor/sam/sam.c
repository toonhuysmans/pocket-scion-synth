...
char __scratch_x("sam_work") input[256]; //tab39445
//standard sam sound
...
unsigned char __scratch_x("sam_work") stress[256]; //numbers from 0 to 8
unsigned char __scratch_x("sam_work") phonemeLength[256]; //tab40160
unsigned char __scratch_x("sam_work") phonemeindex[256];

unsigned char __scratch_x("sam_work") phonemeIndexOutput[60]; //tab47296
unsigned char __scratch_x("sam_work") stressOutput[60]; //tab47365
unsigned char __scratch_x("sam_work") phonemeLengthOutput[60]; //tab47416
...
sam_output_writer_t output_writer = NULL;
...
void SetOutputBuffer(char *_buffer, int capacity)
{
    buffer = _buffer;
    buffer_capacity = capacity;
}
void SetOutputWriter(sam_output_writer_t writer) { output_writer = writer; }
...
void PrepareOutput()
{
    ...
    {
        A = phonemeindex[X];
        // The renderer staging arrays contain 60 entries. Long or heavily
        // punctuated phrases can produce more than 59 short phonemes before
        // InsertBreath adds its normal chunk marker. Render a bounded chunk
        // and reprocess the current phoneme instead of overwriting the
        // adjacent SAM work arrays.
        if (Y >= 59)
        {
            phonemeIndexOutput[59] = 255;
            Render();
            Y = 0;
            if (A == 255) return;
            if (A == 254) { X++; continue; }
...
if (debug) printf("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
    ...
    while(1)
    {
        ...
        if (A != 0)
        {
            ...
            // get flags
            if (index == 255) {
                // There is no following consonant to modify. The original
                // translation assigned a sentinel flag value here, then
                // nevertheless indexed flags[255] below.
                loopIndex++;
                continue;
            }
            mem56 = flags[index];
