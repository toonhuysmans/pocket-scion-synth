...
unsigned char __scratch_y("sam_render") pitches[256]; // tab43008

unsigned char __scratch_y("sam_render") frequency1[256];
unsigned char __scratch_y("sam_render") frequency2[256];
unsigned char __scratch_y("sam_render") frequency3[256];

unsigned char __scratch_y("sam_render") amplitude1[256];
unsigned char __scratch_y("sam_render") amplitude2[256];
unsigned char __scratch_y("sam_render") amplitude3[256];

unsigned char __scratch_y("sam_render") sampledConsonantFlag[256]; // tab44800
...
extern sam_output_writer_t output_writer;

static void WriteSample(int position, unsigned char sample)
{
    if (position < 0) return;
    if (output_writer != 0) output_writer(position, sample);
    else if (buffer != 0 && position < buffer_capacity) buffer[position] = sample;
}
...
void Output8BitAry(int index, unsigned char ary[5])
{
    ...
    for(k=0; k<5; k++)
    {
        int position = bufferpos / 50 + k;
        WriteSample(position, ary[k]);
    }
}
    ...
    do
    {
        ...
        do
        {
            ...
            if ((tempA & 128) != 0)
            {
                ...
                // mem[54296] = X;
                bufferpos += 150;
                WriteSample(bufferpos / 50, (unsigned char)((X & 15)*16));
            } else
            {
                //mem[54296] = 6;
                X=6;
