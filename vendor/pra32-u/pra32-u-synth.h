...
static const uint8_t s_program_table_panel_parameters[] = {
  PANEL_SCALE    ,
  PANEL_TRANSPOSE,
  PANEL_PLAY_MODE,
  PANEL_MIDI_CH  ,
...
if constexpr (BYPASS_FX == false) {
  ...
  // Read-only live modulation taps for platform UI/visualization. Amp EGs are
  // the odd entries in the per-voice filter/amp envelope pairs.
  INLINE int16_t get_amp_envelope_output() {
    int16_t level = m_eg[1].get_output();
    for (uint8_t voice = 1; voice < 4; ++voice) {
      int16_t voice_level = m_eg[(voice << 1) + 1].get_output();
      if (voice_level > level) level = voice_level;
    }
    return level;
  }

  INLINE int16_t get_lfo_output() {
    return m_lfo.get_output();
  }
...
#if defined(PRA32_U_USE_2_CORES_FOR_SIGNAL_PROCESSING) || defined(PRA32_U_EMULATION)
    static const uint8_t voice_mode_table[6] = {
      VOICE_POLYPHONIC,
      VOICE_PARAPHONIC,
      VOICE_MONOPHONIC,
      VOICE_MONOPHONIC,
